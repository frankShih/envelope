...
%   a vector of labels
% output: 
%  a matrix of each users' mean & standard deviation (in label order)
    ...
    for i =1:size(label,1)
        ...
        if size(temp,1)==1 		%use its std to build a brief envelope, when lack of data
            temp_std = 0.01*std(temp)*ones(1, length(temp));     
            temp_mean = temp;                    
        else
